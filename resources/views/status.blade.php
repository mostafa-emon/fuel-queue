    ...
    <div class="max-w-6xl mx-auto">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
            <div>
                ...
                <p class="text-slate-400 mt-2 text-sm md:text-base">Real-time availability for
                    {{ now()->format('d-m-Y') }}
                </p>
            </div>
            <div class="flex gap-3 md:gap-4 w-full md:w-auto">
                <a href="{{ route('login') }}"
                    class="flex-1 md:flex-none glass px-4 md:px-6 py-2 rounded-full hover:bg-white/10 transition text-center text-sm md:text-base">Login</a>
                <a href="{{ route('register') }}"
                    class="flex-1 md:flex-none bg-blue-600 px-4 md:px-6 py-2 rounded-full hover:bg-blue-700 transition text-center text-sm md:text-base">Book
                    Slot</a>
            </div>
        </header>
        ...
        @if(count($stats) > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($stats as $stat)
                    <div
                        class="glass p-6 rounded-3xl card-gradient relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <span
                                    class="text-xs uppercase tracking-widest text-blue-400 font-bold">{{ $stat['fuel'] }}</span>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-1000"
                                    style="width: {{ ($stat['booked'] / $stat['capacity']) * 100 }}%"></div>
                            </div>
